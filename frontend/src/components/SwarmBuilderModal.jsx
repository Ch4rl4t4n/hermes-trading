...
const EXAMPLE_PROMPTS = [
  ...
  { label: "🔧 Site maintenance & monitoring", prompt: "Create a maintenance swarm that monitors uptime, scans logs for anomalies, runs security audits, and triggers recovery tasks automatically." },
  ...
  { label: "🎯 SEO & content", prompt: "Create an SEO and content swarm that researches keywords, drafts article outlines, optimizes pages, and tracks ranking movement." },
];
...
export default function SwarmBuilderModal({ open, onClose, onCreated, onToast }) {
  ...
  const runGenerate = async (inputPrompt) => {
    ...
    try {
      ...
      if (!data?.config) {
        onToast?.("AI did not return a configuration");
      } else {
      ...
      onToast?.(error?.response?.data?.error || "Generation failed");
    } finally {
      setLoadingGenerate(false);
    }
  };
  ...
  const savePrompt = () => {
    ...
    if (!writePromptLibrary(next)) {
      onToast?.("Could not save prompt in this browser");
      return;
    }
  ...
  const createSwarm = async () => {
    ...
    try {
      ...
      onToast?.(`Agents created: ${data?.agents_created || 0}`);
      onCreated?.();
      onClose?.();
    } catch (error) {
    ...
    >
      <article className="modal glass swarm-builder-modal">
        <div className="row between">
          <button type="button" className="share-btn" onClick={onClose} aria-label="Close Swarm Builder">× Close</button>
        ...
        <div className="swarm-builder-grid">
          <section className="swarm-builder-left">
            ...
            <button type="button" className={`swarm-generate-btn ${loadingGenerate ? "is-loading" : ""}`} disabled={loadingGenerate} onClick={() => runGenerate(prompt)}>
              {loadingGenerate ? "AI is drafting your swarm..." : "✨ Generate preview"}
            </button>
            ...
            {config ? (
              <div className="swarm-refine-wrap">
                ...
                <div className="row gap-2">
                  ...
                  <button type="button" className="pill pill-gray" disabled={loadingGenerate || loadingCreate} onClick={savePrompt}>Save prompt</button>
                </div>
              </div>
            ) : null}
          </section>
                ...
                <article className="glass-2 swarm-advanced">
                  ...
                  {showAdvanced ? (
                    <div className="swarm-advanced-body">
                      ...
                      />
                      <label className="fs-12 text-3">Daily cost limit (€)</label>
                      ...
                      <label className="swarm-toggle"><input type="checkbox" checked={Boolean(config.human_approval_required)} onChange={(event) => updateConfig({ human_approval_required: event.target.checked })} /> Require human approval</label>
                    </div>
                  ) : null}
                </article>
